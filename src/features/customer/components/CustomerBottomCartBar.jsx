...
import { ArrowRight, ShoppingBag, ChefHat, CheckCircle2 } from 'lucide-react';
 ...
 * - Khi giỏ rỗng nhưng bàn đã có món gửi bếp: Hiển thị tiến độ món đang chế biến/đã phục vụ.
 */
export default function CustomerBottomCartBar({
  ...
  // 2. Khi giỏ nháp rỗng nhưng bàn đã có món gửi bếp: Hiển thị thanh tiến độ chế biến
  if (orderedCount > 0) {
    const servedCount = orderedCount - activeCookingCount;
    return (
      <footer className="lg:hidden fixed bottom-3 left-3 right-3 z-40 max-w-[calc(100%-1.5rem)] sm:max-w-md md:max-w-lg sm:mx-auto sm:left-0 sm:right-0">
        <div
          onClick={onOpenCart}
        ...
        >
          <div className="flex items-center gap-2.5">
            <div className="w-9 h-9 rounded-xl bg-[#2A1808] border border-amber-500/40 flex items-center justify-center text-amber-400">
              <ChefHat className="w-5 h-5 animate-pulse" />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <span className="text-[11px] font-bold text-[#FFE699]">
                  {activeCookingCount > 0 ? 'Bếp đang chế biến' : 'Tất cả món đã phục vụ'}
                </span>
              ...
              <p className="text-[10px] text-[#A39E93]">
                {activeCookingCount > 0 ? `${activeCookingCount} đang nấu` : 'Sẵn sàng'} • {servedCount} đã phục vụ
              </p>
            </div>
          </div>
